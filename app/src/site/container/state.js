import {action, compute, Service, util} from "adajs";
...
class ContainerService extends Service {
    defaultData() {
        ...
        return {
            ...
            menu: menuinfo,
            current: menuinfo[0],
            currentLeaf: menuinfo[0].list[0].list[0],
            submenu: []
        };
    }
    ...
    open(current, item) {
        ...
        if (target) {
            ...
            current.currentLeaf = target.list[0].list[0];
        }
        return current;
    }
    ...
    flip(current, item) {
        ...
        current.currentLeaf = paths[paths.length - 1];
        return current;
    }
